...
static const char col_names[COLS] = {
    'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h'
};
...
void app_main(void)
{
    ...
    printf("\nTeste isolado da malha de reed switches\n");
    printf("Colunas: saidas digitais do ESP32\n");
    printf("Linhas: entradas digitais com pull-down externo de 10 kOhm para GND\n");
    printf("Diodos: anodo no lado da coluna, catodo/faixa no lado da linha\n");

    while (1) {
        scan_matrix(board);

        if (memcmp(board, last_board, sizeof(board)) != 0) {
            print_board(board);
